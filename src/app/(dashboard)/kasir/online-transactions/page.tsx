...
export default function KasirOnlineTransactions() {
  ...
  useEffect(() => {
    ...
    const channel = supabaseRealtime
      .channel('online_orders_sync_kasir')
    ...
    <div className="space-y-6 pb-24 mx-auto">
      ...
      <div className="space-y-4 mb-10">
        ...
        {unfulfilledOrders.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {unfulfilledOrders.map(o => <OnlineOrderCard key={o.id} transaction={o} role="kasir" onAction={handleAction} />)}
          </div>
        ) : (
      ...
      <div className="space-y-4">
        ...
        {activeKitchenOrders.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {activeKitchenOrders.map(o => <OnlineOrderCard key={o.id} transaction={o} role="kasir" onAction={handleAction} onPrintNota={handlePrintNota} />)}
          </div>
        ) : (
          <div className="bg-white border border-gray-100 rounded-2xl py-12 flex flex-col items-center justify-center text-gray-400 shadow-sm">
